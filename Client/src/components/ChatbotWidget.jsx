				...
				>
					{msg.role === 'assistant' ? (
						<ReactMarkdown
							components={{
								p: ({ children }) => (
									<p style={{ margin: '0 0 7px', color: '#e2e8f0' }}>
										{children}
									</p>
								),
								ul: ({ children }) => (
								...
								strong: ({ children }) => (
									<strong style={{ fontWeight: 700, color: '#4ade80' }}>
										{children}
									</strong>
								),
								em: ({ children }) => (
		...
		const onMove = (e) => {
			...
			if (resizeRef.current) {
				...
				if (handle.includes('w')) {
					nw = Math.min(MAX_W, Math.max(MIN_W, pw - dx));
					nx = px + pw - nw;
				}
				if (handle.includes('n')) {
					nh = Math.min(MAX_H, Math.max(MIN_H, ph - dy));
					ny = py + ph - nh;
				}
				setSize({ w: nw, h: nh });
				setPos({ x: nx, y: ny });
					...
					{isOpen && (
						<motion.div
							...
							style={{
								...
								border:
									mobile || maximized ? 'none' : '1px solid rgba(34,197,94,0.2)',
								boxShadow: mobile
						...
						>
							...
							{!mobile &&
								...
								HANDLES.map((h) => (
									<div
										...
										style={{
											cursor: h.cursor,
											borderRadius: h.id.length === 2 ? 4 : 0,
											...h.style,
										}}
							...
							<div
								...
								style={{
									...
									background:
										'linear-gradient(135deg, #050e08 0%, #071508 50%, #060d1b 100%)',
									display: 'flex',
									alignItems: 'center',
									...
									<div style={{ position: 'relative', flexShrink: 0 }}>
										<div
											style={{
												...
												color: '#22c55e',
												boxShadow:
													'0 0 16px rgba(34,197,94,0.3), inset 0 1px 0 rgba(34,197,94,0.1)',
											}}
										>
											<BotIcon size={20} />
										</div>
									...
									<div>
										<div
											style={{
												...
												letterSpacing: 0.1,
												background:
													'linear-gradient(90deg, #ffffff 0%, #d1fae5 60%, #22c55e 100%)',
